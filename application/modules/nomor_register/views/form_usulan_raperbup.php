                ...
                <div class="card-body">
                    ...
                    <div class="form-group row" id="tembusan-section">
                        ...
                        <div class="col-lg-10 d-flex">
                            ...
                            <div class="revisi-container" id="revisi-tembusan"></div>
                        </div>
                    </div>
...
<script>
    ...
    function tampilkanRevisi() {
        ...
        dataRevisi.forEach(revisi => {
            const key = revisi.kolom_tujuan === 'memutuskan' ? `memutuskan-${revisi.id_revisi}` : revisi.kolom_tujuan;
            if (!revisiMap[key]) revisiMap[key] = [];
            revisiMap[key].push(revisi);
        });

        // Menampilkan revisi untuk setiap kolom
        for (const [key, revisiList] of Object.entries(revisiMap)) {
            let html = '<div class="revisi-list"><h6>Catatan Revisi:</h6><ul>';
            revisiList.forEach(revisi => {
                const namaUser = revisi.nama_user || `User ${revisi.id_user}`; // Ganti dengan logika pengambilan nama user dari tabel users jika ada
                const tanggalUpdate = formatTanggalIndonesia(revisi.updated_at);
                html += `<li>${revisi.catatan_revisi} <br><small>Terakhir diubah oleh: ${namaUser} pada ${tanggalUpdate}</small></li>`;
            });
            html += '</ul></div>';
            $(`#revisi-${key}`).html(revisiList.length > 0 ? html : '');
        }
    }
    ...
    function addKeputusanField(number) {
        ...
        tampilkanRevisi(); // Perbarui revisi setelah menambah keputusan
    }
